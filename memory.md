...
- Screenshot automation is owned by `scripts/nexus-engine-editor-screenshot-mcp.mjs`; it installs NexusEngine Core's optional `mcp-registry-kit` and uses the Core MCP Node adapter with the official SDK's stdio transport. Only its seven explicit Editor tools, capability resource, and review prompt are exposed; file-writing tools fail closed unless `NEXUS_EDITOR_MCP_ALLOW_WRITES=1`.

...
- The browser editor targets NexusEngine commit `a68544434424438491be1398e3f3d5aced5bc5ee`, loads the root and Composition semantic module separately, and resolves executable factories only through canonical package exports. A failed immutable import does not grant a project registry executable trust.
...
- The Engine dependency is pinned to commit `a68544434424438491be1398e3f3d5aced5bc5ee`; `package.json` also records registry and packed-artifact hashes. Validation uses the exact installed tarball, never a sibling source import or symlink.

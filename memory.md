...
- Screenshot and Composition automation are owned by `scripts/nexus-engine-editor-screenshot-mcp.mjs`; it installs NexusEngine Core's optional MCP and Composition Kits and uses the Core MCP Node adapter with the official SDK's stdio transport. It exposes seven Editor tools plus the six Core Composition tools; file-writing tools and `composition_apply` fail closed unless `NEXUS_EDITOR_MCP_ALLOW_WRITES=1`.
- The CLI/MCP dependency is pinned to an exact NexusEngine `0.0.4` commit and must pass from a normal installed package, not a sibling source import or symlink. The browser remains on its separately documented static `0.0.3` CDN fallback until a browser cutover is explicitly approved.

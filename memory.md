...
- `src/editor-composition.js` owns project-format `0.3.0` migration, the staged registry-reference tree, atomic Apply, reference protection, trusted factory resolution, disposable Run Once previews, and disposable Play instances.
...
- Project format `0.3.0` stores accepted `composition` plus a project-local registry overlay. `domainStack`, `kitConfigs`, object `domainKits`, and `domainStackHealth` are derived compatibility projections for existing templates, CLI commands, and HTML exports.
...
- Run Once and Play resolve only trusted exports from the already-loaded sibling/Core NexusEngine module. Imported manifest-only kits report preview unavailable. A preview command runs only when the trusted descriptor marks a simple command name `editorSafe`; async commands have bounded timeouts, otherwise Run Once performs one fixed `1/60` tick. Every preview uses and disposes a fresh Engine and retains at most 20 JSON-safe receipts.
...
- Screenshot automation is owned by `scripts/nexus-engine-editor-screenshot-mcp.mjs`; it installs the optional NexusEngine MCP Domain Service Kit and uses the official SDK's stdio transport. Only its seven explicit Editor tools, capability resource, and review prompt are exposed; file-writing tools fail closed unless `NEXUS_EDITOR_MCP_ALLOW_WRITES=1`.
...
- Editors and games receive no MCP surface by default. A runtime must explicitly install the MCP DSK and register its own provider; the example game exposes only status, bounded stepping, state, and one prompt, with actions gated by `NEXUS_GAME_MCP_ALLOW_ACTIONS=1`.

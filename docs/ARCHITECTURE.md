...
Project format `0.3.0` stores an accepted `nexusengine.composition-tree/1` tree
and a project-local registry overlay. Legacy `domainStack`, `kitConfigs`, and
object `domainKits` are derived compatibility projections.
...
The public browser pin is NexusEngine `0.0.3`; composition-tree editing requires
newer registry-v2 APIs. Node tooling uses the exact `0.0.4` revision in the
lockfile. The split is deliberate until a browser compatibility cutover is
